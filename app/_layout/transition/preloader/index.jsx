...
/** Turns "/work" into "Work" for the loader on non-home routes. */
function routeLabel(pathname) {
  return pathname.slice(1).replace(/^./, letter => letter.toUpperCase());
}
...
export function Preloader() {
  ...
  useTimeOut({
    callback: () => {
      setIndex(prevIndex => Math.min(prevIndex + 1, preloaderWords.length - 1));
    },
      ...
      {width > 0 ? (
        <>
          ...
          >
            ...
            <p>
              {pathname === '/' ? preloaderWords[index] : routeLabel(pathname)}
            </p>
          </MotionComponent>
